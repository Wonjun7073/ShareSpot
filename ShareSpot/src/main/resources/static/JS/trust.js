...
(function () {
    ...
    async function main() {
        ...
        // ===== 상단 점수/퍼센트/게이지 =====
        const score = clamp(me?.trustScore ?? 0, 0, 500);
        const percent = Math.round((score / 500) * 100);

        percentEl.textContent = `${percent}%`;
        gradeEl.textContent = `신뢰 지수 - ${gradeByPercent(percent)}`;
        barEl.style.width = `${percent}%`;
        scoreEl.textContent = `총 ${score}점 / 500점`;
        ...
        if (cntEl && ptEl) {
            ...
            if (completedAsSeller <= 0) {
                // ✅ 구매자는 여기서 0이므로 아예 숨김
                const row = cntEl.closest(".activity-item") || cntEl.closest(".activity-row");
                if (row) row.style.display = "none";
            } else {
                cntEl.textContent = `${completedAsSeller}회`;
                ptEl.textContent = `+${completedAsSeller * 10}점`;
            }
        }
    }
